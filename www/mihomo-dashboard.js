...
class MihomoDashboard extends HTMLElement {
  ...
  _render() {
    ...
    const integrationVersion = this._state(this._config.primary)?.attributes?.integration_version;
    const versionBadge = integrationVersion
      ? `<span class="version-badge" title="Версия интеграции Clash Controller">v${esc(integrationVersion)}</span>`
      : '<span class="version-badge is-unavailable" title="Версия появится после обновления интеграции">v—</span>';
    ...
    this.innerHTML = `<style>${MihomoDashboard.styles}</style>
      <main class="shell">
        <header class="topbar">
          <div class="brand"><span class="brand-mark">◈</span><div><span class="overline">CUDY ROUTER</span><div class="brand-title"><h1>Proxy control</h1>${versionBadge}</div></div></div>
        ...
        <footer><span>Clash Controller · локальное управление</span><span>API через Home Assistant</span></footer>
